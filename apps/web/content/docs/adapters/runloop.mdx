...
<Callout type="info">
  ...
  through a shell, so the core folds `cwd`/`env` into a `cd … && KEY=v …` wrapper
  (`perCommandEnvCwd: false`). Files use the binary-safe `downloadFile` /
  `uploadFile` endpoints. `suspend`/`resume` map to `pause()`/`resume()` (memory
  preserved); `shutdown` is `destroy()`. `snapshots.create()` is `snapshotDisk`
  and `fork()` snapshots then boots N devboxes — in-place `restore` is not
  supported (snapshots create new devboxes).
</Callout>
