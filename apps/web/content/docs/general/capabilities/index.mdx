...
- `emulated` — supported, but polyfilled by the core (e.g. exec-based filesystem ops, or [live streaming](/docs/api/commands#streaming-is-a-guarantee-not-a-per-provider-feature) via an in-sandbox tail).
- `unsupported` — not available; the sub-API is typed `undefined` and throws before any network call.

`emulated` is a statement about _who implements it_, not about whether it works. An `emulated` capability behaves like the real thing; the core just supplies it.
