...
A single TypeScript API for running code in ephemeral **sandboxes** that works the same way across [E2B](/adapters/e2b), [Vercel Sandbox](/adapters/vercel), [Cloudflare Sandbox](/adapters/cloudflare), [Daytona](/adapters/daytona), [Modal](/adapters/modal), [Fly Machines](/adapters/fly), [AWS Lambda MicroVMs](/adapters/aws-lambda), and an in-memory provider for tests — with more adapters on the way (Northflank, Runloop, CodeSandbox, Morph).
